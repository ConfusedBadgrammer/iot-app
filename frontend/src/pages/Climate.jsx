...
function Climate() {
  ...
  const dewPoint = sensorData ? calcDewPoint(sensorData.temperature, sensorData.humidity) : null
  ...
  useEffect(() => {
    setSensorData(null)
    fetch(`/api/v1/sensor/${selectedMac}/latest`)
      .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/sensor/${selectedMac}?from=${from}&to=${to}`)
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="flex flex-col gap-6 flex-1">
      <div className="flex justify-between">
        <div className="flex flex-col">
          <div className="flex flex-col pb-8">
            <h1 className="font-bold text-5xl antialiased font-headline pb-1">Climate</h1>
            {sensorData && (
          ...
          <SubNav devices={devices} selected={selectedMac} onSelect={setSelectedMac} />
        </div>
      ...
      <div className="flex gap-6 w-full">
        <Card className="flex-1 min-w-64 min-h-72">
          <CardTitle label="AMBIENT TEMPERATURE" icon={<Thermometer size={30} />}>
            Thermal Balance
          </CardTitle>
          <CardBody className="flex justify-center items-center flex-1 flex-col">
            <div className="text-6xl flex gap-2">
              ...
              <div className="flex flex-col-reverse">
                ...
                {sensorData && oneHrSensorData && (() => {
                  const diff = sensorData.temperature - oneHrSensorData.temperature
                  return (
                    <span className={`text-sm ${diff < 0 ? 'text-(--color-accent)' : 'text-red-400'}`}>
                      {diff.toFixed(2)}
                    </span>
                  )
                })()}
          ...
          <ProgressBar
            ...
            max={weatherData?.temperature ? weatherData.temperature.degrees * 2 : 30}
          ...
          <CardBody className="flex justify-center items-center flex-1 flex-col">
            <div className="text-6xl flex gap-2">
              ...
              <div className="flex flex-col-reverse">
                ...
                {sensorData && oneHrSensorData && (() => {
                  const diff = sensorData.humidity - oneHrSensorData.humidity
                  return (
                    <span className={`text-sm ${diff < 0 ? 'text-(--color-accent)' : 'text-red-400'}`}>
                      {diff.toFixed(2)}
                    </span>
            ...
            <Card className="flex-1">
              ...
              <div className="flex flex-1 items-center justify-center">
                <CircularProgressBar
                  ...
                  strokeWidth={10}
                  size={180}
                  diff={dewPoint != null && oneHrDewPoint != null ? dewPoint - oneHrDewPoint : null}
                />
              </div>
            </Card>
          </div>
